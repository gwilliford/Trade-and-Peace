...
chisols <- read_dta('CHISOLSstyr4_0.dta')
chisols <- dplyr::select(chisols, ccode, year, totalldrtrans_chal = totalldrtrans, leadertrans_chal = leadertrans, solschange_chal = solschange, solschdum_chal = solschdum, solschange30_chal = solschange30, solsch30dum_chal = solsch30dum, solsminchange_chal = solsminchange, solsminchdum_chal = solsminchdum)3

# Merge Monadic Data
#dmon = dplyr::select(gdp, ccode, year, gdpcap, lgdpcap)
dmon = full_join(mad, dcap)
dmon = full_join(dmon, dplyr::select(dpol, ccode, year, polity2))
dmon = full_join(dmon, dplyr::select(dw, ccode, year, W, S))
...
dmon1 = dmon %>% dplyr::select(-"version") %>% setNames(paste0(names(.), "1")) %>% rename(year = year1)
dmon2 = dmon %>% dplyr::select(-"version") %>% setNames(paste0(names(.), "2")) %>% rename(year = year2)
...
icow_full_cyr$ccode1 <- with(icow_full_cyr, rowMins(cbind(chal, tgt)))
icow_full_cyr$ccode2 <- with(icow_full_cyr, rowMaxs(cbind(chal, tgt)))
icow_full_dyr = icow_full_cyr %>% group_by(dyad, year) %>% 
  summarize(
    dyterrclaim = 1, 
    ccode1 = max(ccode1), 
    ccode2 = max(ccode2)
...
### Merge dyadic data
ddy <- dplyr::select(dtrade, ccode1, ccode2, dyad, year, flow1, flow2, smoothflow1, smoothflow2, smoothtotrade)
ddy <- full_join(ddy, dplyr::select(ddist, ccode1, ccode2, dyad, year, ccdistance, mindistance))
ddy <- full_join(ddy, dplyr::select(dcont, ccode1, ccode2, dyad, year, conttype))
ddy <- full_join(ddy, icow_full_dyr)
ddy <- full_join(ddy, dplyr::select(dally, ccode1, ccode2, dyad, year, defense))#neutrality, nonaggression, entente
