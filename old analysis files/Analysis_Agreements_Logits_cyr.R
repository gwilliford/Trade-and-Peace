...
att_dy <- firthlogit::firth(attanyp ~ lterugtdepmax + lag_ln_trade + lag_ln_deptotmax + 
                lag_ln_gdp_chal + lag_ln_gdp_tgt + lag_ln_gdpcap_chal + lag_ln_gdpcap_tgt + 
                lpol_chal  * lpol_tgt +
                icowsal + 
                recnowt + recyeswt + recmidwt +
                lcaprat + ldefense + contdir + 
                year, family = binomial("logit"), data = icow_part_cyr_ct); summary(att_dy)

att_dy <- brglm(agreeiss ~ lterugtdepmax + lag_ln_trade + lag_ln_deptot_chal + lag_ln_deptot_tgt + 
                  lag_ln_gdp_chal + lag_ln_gdp_tgt + lag_ln_gdpcap_chal + lag_ln_gdpcap_tgt + 
                  W_chal  * W_tgt +
                  icowsal + 
                  recnowt + recyeswt + recmidwt +
